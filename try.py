...
def load_knowledge_base(csv_path="C:\\Users\\KentzChristian(Kentz\\Documents\\BS-LangChain-Miguelv2\\BookStop\\dataset\\KB.csv"):
    # Load CSV file
...
def query_vector_store(query, embeddings):
    ...
    # Rerank results
    reranked = rerank_documents(query, hybrid_results)
...
Contains keywords such as: summary, review, recommend, suggest, borrow, return, rent, check out, renew, extend, reserve.
...
branch_chain = RunnableBranch(
    (lambda x: "Not Book-Related" in x["is_book_related"] , off_topic_answer_runnable),
...
def respond(message, chat_history):
    try:
        ...
        if classification.get("response"):
            ...
            recent_history = chat_mem.get("chat_history", [])[-4:]
            chat_history_str = "\n".join(
...
# Build the Gradio UI
with gr.Blocks(title="BookStop Library Assistant 📚", fill_height=True, fill_width=True) as demo:
    gr.Markdown("# BookStop Library Assistant 🤖📚")
    gr.Markdown("Your library companion for book recommendations, availability checks, and bookish chats!")
   
    # Use ChatInterface but always overwrite with only the latest exchange
    chatbot = gr.ChatInterface(
        fn=respond,             # The function that processes each user message
        examples=[
