...
# Individual 
format_prompt = RunnableLambda(lambda x: prompt_template1.format_prompt(**x))
...
chain_verifyQuery = format_prompt | model
...
chain_classifyQuery = format_prompt2 | model
...
qc_Branch = RunnableBranch(
    ...
    (
        lambda x: "BRQ" in x.content,
        RunnableLambda(lambda x: (print(x), chain_classifyQuery))
    ),
    # Branch 2: Unrelated Query (UQ)
    (
        lambda x: "UQ" in x.content,
        RunnableLambda(lambda x: "I'm sorry, let us not go beyond the topic of books and this library!")
    ),
    # Branch 3: End Conversation (EC)
    (
        lambda x: "EC" in x.content,
        RunnableLambda(lambda x: "Got it! Feel free to ask me other questions at your convenience. Thank you for using this chatbot. 'Till next time!")
    ),
    # Default branch (if none of the above conditions are met)
    RunnableLambda(lambda x: "I'm not sure how to handle that. Can you clarify?")
)
